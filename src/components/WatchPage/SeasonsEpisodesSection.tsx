...
const SeasonsEpisodesSection: React.FC<SeasonsEpisodesSectionProps> = ({ tvShow, onSeasonEpisodeChange }) => {
    ...
    const fetchSeasons = async () => {
      try {
        ...
        if (seasonsData && seasonsData.length > 0) {
          const firstSeason = seasonsData.find((season: any) => season.season_number >= 0) || seasonsData[0];
          await handleSeasonChange(firstSeason);
        }
      } catch (error) {
  ...
  const handleSeasonChange = async (season: any) => {
    try {
      ...
      // Notify parent component of season change (default to episode 1)
      if (onSeasonEpisodeChange) {
        onSeasonEpisodeChange(season.season_number, 1);
      }
    } catch (error) {
                    ...
                    <div className="p-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <button
                            onClick={() => toggleEpisodeWatched(episodeId)}
